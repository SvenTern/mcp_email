...
export function taskTools(server: McpServer, bitrixService: BitrixService): void {
  // Create task
  server.registerTool('bitrix_task_create', {
    ...
    if (params.createdBy) fields.CREATED_BY = params.createdBy;
    ...
    if (params.startDatePlan) fields.START_DATE_PLAN = params.startDatePlan;
    if (params.endDatePlan) fields.END_DATE_PLAN = params.endDatePlan;
    ...
    if (params.timeEstimate) fields.TIME_ESTIMATE = params.timeEstimate;
    ...
    const result = await bitrixService.createTask(fields as any);
  ...
  server.registerTool('bitrix_task_update', {
    ...
    const result = await bitrixService.updateTask(params.taskId, fields as any);
  ...
  server.registerTool('bitrix_task_list', {
    ...
    if (params.responsibleId) filter.RESPONSIBLE_ID = params.responsibleId;
    if (params.createdBy) filter.CREATED_BY = params.createdBy;
    if (params.groupId) filter.GROUP_ID = params.groupId;
    if (params.parentId) filter.PARENT_ID = params.parentId;
    if (params.status) filter.STATUS = params.status;
    if (params.priority !== undefined) filter.PRIORITY = params.priority;
    if (params.titleSearch) filter['%TITLE'] = params.titleSearch;
    if (params.deadlineFrom) filter['>=DEADLINE'] = params.deadlineFrom;
    if (params.deadlineTo) filter['<=DEADLINE'] = params.deadlineTo;
  ...
  server.registerTool('bitrix_task_get_fields', {
    ...
    categories.other = fieldNames.filter(f =>
      !categories.basic.includes(f) &&
