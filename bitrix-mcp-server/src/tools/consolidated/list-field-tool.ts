...
export function registerListFieldTool(server: McpServer, bitrixService: BitrixService): void {
...
- types: Get available field types (requires iblockTypeId)`,
    ...
    switch (action) {
      case 'add': {
        ...
        if (params.defaultValue) fields.DEFAULT_VALUE = params.defaultValue;
      ...
      case 'get': {
        const fieldsResult = await bitrixService.getListFields(
          iblockTypeId as IblockTypeId,
          params.iblockId,
          params.iblockCode,
          params.fieldId,
