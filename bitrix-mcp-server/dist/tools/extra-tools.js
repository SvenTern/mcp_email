import { z } from 'zod';
export function extraTools(server, bitrixService) {
    // Test connection
    ...
    server.registerTool('bitrix_users_get', {
        ...
        if (params.nameSearch)
            filter.NAME = `%${params.nameSearch}%`;
        const users = await bitrixService.getUsers(Object.keys(filter).length > 0 ? filter : undefined);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
...
//# sourceMappingURL=extra-tools.js.map