...
export function registerUserTool(server, bitrixService) {
...
- by_department: Get users by department (requires departmentId)`,
        ...
        switch (action) {
            case 'list': {
                const result = await bitrixService.getUsersFiltered(params.filter, undefined, undefined, undefined, params.start);
                return {
                    content: [{
                            type: 'text',
                            text: JSON.stringify({
