...
export function registerListSectionTool(server, bitrixService) {
...
- delete: Delete section (requires iblockTypeId, sectionId or sectionCode)`,
        ...
        switch (action) {
            case 'add': {
                ...
                const sectionId = await bitrixService.createListSection(iblockTypeId, params.sectionCode, {
                    NAME: params.name,
                    SORT: params.sort,
                    ACTIVE: params.active === false ? 'N' : 'Y',
                }, params.iblockId, params.iblockCode, params.parentSectionId, params.socnetGroupId);
                return { content: [{ type: 'text', text: JSON.stringify({ success: true, sectionId }) }] };
            }
            case 'get': {
                const result = await bitrixService.getListSections(iblockTypeId, params.iblockId, params.iblockCode, params.filter, params.select, params.socnetGroupId);
                return {
                    content: [{
                            type: 'text',
                            text: JSON.stringify({
            ...
            case 'update': {
                ...
                const success = await bitrixService.updateListSection(iblockTypeId, params.sectionId, params.sectionCode, Object.keys(fields).length > 0 ? fields : undefined, params.iblockId, params.iblockCode, params.socnetGroupId);
            ...
            case 'delete': {
                ...
                const success = await bitrixService.deleteListSection(iblockTypeId, params.sectionId, params.sectionCode, params.iblockId, params.iblockCode, params.socnetGroupId);
                return { content: [{ type: 'text', text: JSON.stringify({ success }) }] };
            }
            default:
...
//# sourceMappingURL=list-section-tool.js.map