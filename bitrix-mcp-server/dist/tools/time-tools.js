import { z } from 'zod';
export function timeTools(server, bitrixService) {
    // Add time entry
    server.registerTool('bitrix_time_add', {
        ...
        if (params.dateStart)
            fields.DATE_START = params.dateStart;
        if (params.dateStop)
            fields.DATE_STOP = params.dateStop;
        const entryId = await bitrixService.addElapsedTime(params.taskId, fields);
    ...
    server.registerTool('bitrix_time_update', {
        ...
        if (params.comment)
            fields.COMMENT_TEXT = params.comment;
        await bitrixService.updateElapsedTime(params.taskId, params.entryId, fields);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
...
//# sourceMappingURL=time-tools.js.map