import { z } from 'zod';
export function departmentTools(server, bitrixService) {
    ...
    server.registerTool('bitrix_department_list', {
        ...
        if (params.headId !== undefined)
            filter.UF_HEAD = params.headId;
        const result = await bitrixService.getDepartments(Object.keys(filter).length > 0 ? filter : undefined, params.sort, params.order, params.start);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
    ...
    server.registerTool('bitrix_department_employees', {
        ...
        const department = await bitrixService.getDepartmentById(params.departmentId);
        const employees = await bitrixService.getDepartmentEmployees(params.departmentId, params.recursive, params.activeOnly);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
...
//# sourceMappingURL=department-tools.js.map