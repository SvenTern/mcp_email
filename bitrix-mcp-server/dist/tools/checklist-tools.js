import { z } from 'zod';
export function checklistTools(server, bitrixService) {
    // Add checklist item
    server.registerTool('bitrix_checklist_add', {
        ...
        if (params.sortIndex !== undefined)
            fields.SORT_INDEX = params.sortIndex;
        if (params.isComplete !== undefined)
            fields.IS_COMPLETE = params.isComplete ? 'Y' : 'N';
        if (params.parentId)
            fields.PARENT_ID = params.parentId;
        const itemId = await bitrixService.addChecklistItem(params.taskId, fields);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
    ...
    server.registerTool('bitrix_checklist_update', {
        ...
        if (params.sortIndex !== undefined)
            fields.SORT_INDEX = params.sortIndex;
        await bitrixService.updateChecklistItem(params.taskId, params.itemId, fields);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
