import { z } from 'zod';
export function userTools(server, bitrixService) {
    ...
    server.registerTool('bitrix_user_list', {
        ...
        if (params.departmentId)
            filter.UF_DEPARTMENT = params.departmentId;
        if (params.userType)
            filter.USER_TYPE = params.userType;
        if (params.isOnline !== undefined)
            filter.IS_ONLINE = params.isOnline ? 'Y' : 'N';
        const result = await bitrixService.getUsersFiltered(Object.keys(filter).length > 0 ? filter : undefined, params.select, params.sort, params.order, params.start);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
    ...
    server.registerTool('bitrix_user_by_department', {
        ...
        const users = await bitrixService.getDepartmentEmployees(params.departmentId, params.recursive, params.activeOnly);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
