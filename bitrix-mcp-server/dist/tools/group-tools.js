import { z } from 'zod';
export function groupTools(server, bitrixService) {
    // Get list of groups
    server.registerTool('bitrix_group_list', {
        ...
        if (params.visible !== undefined)
            filter.VISIBLE = params.visible ? 'Y' : 'N';
        if (params.opened !== undefined)
            filter.OPENED = params.opened ? 'Y' : 'N';
        if (params.project !== undefined)
            filter.PROJECT = params.project ? 'Y' : 'N';
        if (params.active !== undefined)
            filter.ACTIVE = params.active ? 'Y' : 'N';
        const result = await bitrixService.getGroups(Object.keys(filter).length > 0 ? filter : undefined, params.start);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
    ...
    server.registerTool('bitrix_group_search', {
        ...
        if (params.visible !== undefined)
            filter.VISIBLE = params.visible ? 'Y' : 'N';
        const result = await bitrixService.getWorkgroups(Object.keys(filter).length > 0 ? filter : undefined, undefined, params.start);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
